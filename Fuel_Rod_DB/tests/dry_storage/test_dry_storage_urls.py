...
@pytest.mark.django_db
def test_url_delete(client, user, user_admin, dry_storage_test_factory):
    """
    Fresh_inventory:create test
    """
    rod = dry_storage_test_factory(_quantity=2)
    url_1 = reverse('dry_storage:delete', args=[rod[0].pk])
    url_2 = reverse('dry_storage:delete', args=[rod[1].pk])
    # response for anonymous user
    ...
    assert not RodDryStorageTest.objects.filter(id=rod[1].pk).exists()